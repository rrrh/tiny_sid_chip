`timescale 1ns / 1ps
//==============================================================================
// Testbench for tt_um_sid (1 MHz, SID-compatible waveform layout)
//==============================================================================
...
module tt_um_sid_tb;
    ...
    // SID-compatible waveform constants
    ...
    task measure_pcm;
        input integer num_samples; output integer avg;
        integer sum, got;
        begin
            sum = 0; got = 0;
            while (got < num_samples) begin
                @(posedge clk);
                if (pcm_valid) begin sum = sum + pcm_out; got = got + 1; end
            end
            avg = sum / num_samples;
        end
    endtask
    ...
    initial begin
        ...
        test_num = test_num + 1;
        if (cnt1 > 10) begin $display("TEST %0d PASS: saw pdm=%0d", test_num, cnt1); pass_count = pass_count + 1; end
        ...
        count_pdm(5_000, cnt1);
        test_num = test_num + 1;
        if (cnt1 > 10) begin $display("TEST %0d PASS: tri pdm=%0d", test_num, cnt1); pass_count = pass_count + 1; end
        ...
        count_pdm(5_000, cnt1);
        test_num = test_num + 1;
        if (cnt1 > 10) begin $display("TEST %0d PASS: pulse pdm=%0d", test_num, cnt1); pass_count = pass_count + 1; end
        ...
        count_pdm(5_000, cnt1);
        test_num = test_num + 1;
        if (cnt1 > 10) begin $display("TEST %0d PASS: noise pdm=%0d", test_num, cnt1); pass_count = pass_count + 1; end
        ...
        sid_write(REG_PW, 8'h80, 2'd2);
        sid_write(REG_ATK, 8'h00, 2'd2); sid_write(REG_SUS, 8'h0F, 2'd2);
        sid_write(REG_WAV, PULSE | GATE, 2'd2);
        repeat (50_000) @(posedge clk);
        count_pdm(5_000, cnt1);
        test_num = test_num + 1;
        if (cnt1 > 10) begin $display("TEST %0d PASS: 3-voice pdm=%0d", test_num, cnt1); pass_count = pass_count + 1; end
    ...
    initial begin #10_000_000_000; $display("\nERROR: Timeout!"); $finish; end
